 ...
 */
const Instructions = () => {
  const { selectedCategory, selectedDifficulty, startTest, questions } = useTest()
  ...
  // Redirect if no test is configured
  if (!selectedCategory || !selectedDifficulty || questions.length === 0) {
    navigate('/dashboard')
    return null
  }
  ...
  const instructions = [
    {
      ...
      description: `This test contains ${TEST_CONFIG.TOTAL_QUESTIONS} multiple choice questions.`
    },
    {
      icon: <FiClock className="w-5 h-5" />,
      title: 'Time Limit',
      description: `You have ${TEST_CONFIG.TIME_LIMIT_MINUTES} minutes to complete the test. The timer starts when you click "Start Test".`
    },
    {
          ...
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-8 text-white">
            ...
            <div className="flex flex-wrap items-center gap-4 text-white/90">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium capitalize ${
                difficulty?.id === 'easy' ? 'bg-green-500/30' :
                difficulty?.id === 'medium' ? 'bg-yellow-500/30' :
                difficulty?.id === 'hard' ? 'bg-red-500/30' : 'bg-purple-500/30'
              }`}>
                {difficulty?.name}
              </span>
            </div>
