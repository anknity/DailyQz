...
import { 
  ...
  FiRotateCcw, FiHome, FiChevronDown, FiChevronUp, FiEye, FiShare2, FiAward
} from 'react-icons/fi'
...
const SchoolResult = () => {
  ...
  useEffect(() => { if (!result) navigate('/schools') }, [result])
  if (!result) return null

  const { questions, answers, correct, incorrect, unanswered, total, score, timeTaken, testTitle, testConfig } = result
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${score >= 50 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{score >= 50 ? 'PASSED' : 'FAILED'}</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-white">{testTitle || 'School Exam'}</h1>
            <p className="text-gray-400 text-sm mt-1">Completed just now &bull; {formatTime(timeTaken)} Duration</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8">
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: 'spring' }} className="text-4xl font-extrabold text-white">{score}%</motion.span>
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-gray-400 text-sm mb-5 leading-relaxed">{score >= 70 ? 'Your performance has been exceptional. Keep up the great work!' : 'Keep practicing to improve your performance.'}</p>
                <div className="grid grid-cols-3 gap-6">
                  <div><span className="text-gray-500 text-xs block mb-1">Correct</span><span className="text-xl font-bold text-green-400">{correct}</span></div>
                  <div><span className="text-gray-500 text-xs block mb-1">Incorrect</span><span className="text-xl font-bold text-red-400">{incorrect}</span></div>
                  <div><span className="text-gray-500 text-xs block mb-1">Skipped</span><span className="text-xl font-bold text-gray-400">{unanswered}</span></div>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-[#111d2e] rounded-2xl p-6 border border-gray-700/30 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1"><FiAward className="w-5 h-5 text-amber-400" /><h3 className="text-lg font-bold text-white">Grade Report</h3></div>
              <p className="text-gray-500 text-sm mb-4">{testConfig?.subject || 'Subject'} Performance</p>
            </div>
            <div>
              <div className="text-center py-3">
                <span className="text-5xl font-extrabold" style={{ color: scoreColor }}>{score >= 90 ? 'A+' : score >= 80 ? 'A' : score >= 70 ? 'B+' : score >= 60 ? 'B' : score >= 50 ? 'C' : 'D'}</span>
              </div>
              <div className="w-full h-2 bg-gray-800 rounded-full mt-3 overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${score}%` }} transition={{ delay: 0.6, duration: 1 }} className="h-full rounded-full" style={{ background: `linear-gradient(to right, ${scoreColor}88, ${scoreColor})` }} /></div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-[#111d2e] rounded-2xl p-5 border border-gray-700/30">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between text-xs text-gray-500 mb-1.5"><span>Your Avg: {avgTimePerQ}s</span><span>Total: {formatTime(timeTaken)}</span></div>
              <div className="flex gap-1.5 h-2.5"><div className="bg-amber-500 rounded-full" style={{ width: `${Math.min(100, (avgTimePerQ / 90) * 100)}%` }} /><div className="flex-1 bg-amber-900/40 rounded-full" /></div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-[#111d2e] rounded-2xl p-5 border border-gray-700/30">
            ...
            <div className="space-y-3">
              {[{ name: 'Correct', pct: total > 0 ? Math.round((correct / total) * 100) : 0, color: 'bg-green-500' },
                { name: 'Incorrect', pct: total > 0 ? Math.round((incorrect / total) * 100) : 0, color: 'bg-red-500' },
                { name: 'Skipped', pct: total > 0 ? Math.round((unanswered / total) * 100) : 0, color: 'bg-gray-500' }
              ].map((t, i) => (
                  ...
                  {questions.map((question, index) => {
                    ...
                    return (
                      <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.03 }}
                        className={`bg-[#111d2e] border rounded-xl p-4 border-l-4 ${isCorrect ? 'border-green-500/50 border-l-green-500' : !isUnanswered ? 'border-red-500/50 border-l-red-500' : 'border-gray-700/50 border-l-gray-600'}`}>
                        <div className="flex items-start gap-4">
                          <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${isCorrect ? 'bg-green-500/20 text-green-400' : !isUnanswered ? 'bg-red-500/20 text-red-400' : 'bg-gray-700 text-gray-400'}`}>
                            {isCorrect ? <FiCheck /> : !isUnanswered ? <FiX /> : '-'}
                          </span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-gray-500 text-xs font-medium">Q{index + 1}</span>
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isCorrect ? 'bg-green-500/20 text-green-400' : !isUnanswered ? 'bg-red-500/20 text-red-400' : 'bg-gray-700 text-gray-400'}`}>
                                {isCorrect ? 'Correct' : !isUnanswered ? 'Wrong' : 'Skipped'}
                              </span>
                            </div>
                            ...
                            <div className="space-y-2">
                              {question.options.map((option, optIndex) => {
                                ...
                                return (
                                  <div key={optIndex} className={`flex items-center gap-3 p-2.5 rounded-lg text-sm ${isCorrectOption ? 'bg-green-500/10 border border-green-500/30 text-green-300' : isUserAnswer ? 'bg-red-500/10 border border-red-500/30 text-red-300' : 'bg-gray-800/50 text-gray-400'}`}>
                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${isCorrectOption ? 'bg-green-500 text-white' : isUserAnswer ? 'bg-red-500 text-white' : 'bg-gray-700 text-gray-400'}`}>{String.fromCharCode(65 + optIndex)}</span>
                                    <span className="flex-1">{option}</span>
                                    {isCorrectOption && <FiCheck className="w-4 h-4 text-green-400" />}
                                    {isUserAnswer && !isCorrectOption && <FiX className="w-4 h-4 text-red-400" />}
                                  </div>
                                )
                              })}
                            </div>
                            {question.explanation && (<div className="mt-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg"><p className="text-sm text-blue-300">💡 {question.explanation}</p></div>)}
                          </div>
                        </div>
                      </motion.div>
                    )
        ...
        {/* Action Buttons */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="flex flex-col sm:flex-row gap-4 mt-6">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigate('/schools')}
            ...
            <FiRotateCcw className="w-5 h-5" /> Take Another Test
          </motion.button>
        </motion.div>
      </div>
    </div>
